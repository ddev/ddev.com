...
modifiedComment: "Corrected the TYPO3 settings filename again: current composer-mode TYPO3 (v12+) uses config/system/additional.php, not AdditionalConfiguration.php, which is only written for legacy pre-v12 installs. Also repointed the settings-management docs link at the article its content was absorbed into (usage/cms-settings) instead of the now-unrelated customization-extendibility page."
...
- Create a specialty config file with DDEV-specific settings (like `config/system/additional.php` for current composer-mode TYPO3, or `typo3conf/AdditionalConfiguration.php` for legacy TYPO3 v11 and below, or `settings.ddev.php` for Drupal).
...
- There is also an environment variable `$IS_DDEV_PROJECT` that can be used to fence off DDEV-specific behavior. For example, with `$IS_DDEV_PROJECT` empty, the important parts of `settings.ddev.php` and `config/system/additional.php` (for TYPO3) are not executed. This means that DDEV’s `settings.ddev.php` won’t be invoked if it somehow ends up in a production environment or in a non-DDEV local development environment.
