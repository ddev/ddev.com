...
When you run `ddev stop` from the working directory (or `ddev stop <project1> <project2>` DDEV removes the containers, but it does not remove the data. Check out the docs on [removing a project.](https://docs.ddev.com/en/stable/users/cli-usage/#removing-projects-from-ddev-local)
